    ...
    if (text.startsWith('/start')) {
      ...
      if (refID) {
        try {
          ...
          const { data, error } = await SupabaseClient.from('referral')
            .insert([
              {
                refKey: `${refID}-${message.from.username}`,
                refId: `${refID}`,
                referId: `${message.from.id}`,
              },
            ])
            .select('*')
      ...
      bot.sendPhoto(
        chatID,
        'https://solana-wallet-orcin.vercel.app/assets/new.png',
        {
